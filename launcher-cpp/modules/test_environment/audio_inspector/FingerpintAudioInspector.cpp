...
#include "FingerprintAudioInspector.hpp"

FingerprintAudioInspector::FingerprintAudioInspector(int sample_rate)
    : expected_sample_rate_(sample_rate)
{
}

std::vector<float> FingerprintAudioInspector::to_mono(const audio_chunk& chunk)
{
...
SpectralFingerprint FingerprintAudioInspector::create_fingerprint(const std::vector<float>& signal)
{
...
bool FingerprintAudioInspector::is_silent(const std::vector<float>& signal)
{
...
void FingerprintAudioInspector::cleanup_old_references_()
{
...
void FingerprintAudioInspector::update_error_stats_(const VerificationError error)
{
    stats_.error_counts[error]++;
}


FingerprintAudioInspector::~FingerprintAudioInspector() = default;

void FingerprintAudioInspector::add_reference(const audio_chunk& chunk, uint64_t seq)
{
...
VerificationReport FingerprintAudioInspector::verify(const audio_chunk& chunk)
{
    ...
    {
        report.error = VerificationError::NO_REFERENCES;
    ...
    {
        report.error = VerificationError::EMPTY_TEST_SIGNAL;
        update_error_stats_(report.error);
        return report;
    }
    ...
    {
        report.error = VerificationError::SIGNAL_TOO_QUIET;
        update_error_stats_(report.error);
        return report;
    }
    ...
    {
        report.error = VerificationError::LOW_SIMILARITY;
        report.verified = false;
    }
    else if (best_match_count < SpectralFingerprint::NUM_BANDS / 2)
    {
        report.error = VerificationError::TOO_MANY_DIFFERENCES;
        report.verified = false;
    }
    else
    {
        report.error = VerificationError::NONE;
...
const Stats& FingerprintAudioInspector::get_stats() const
{
    return stats_;
}

void FingerprintAudioInspector::reset_stats()
{
    stats_ = Stats{};
}

void FingerprintAudioInspector::clear_references()
{
