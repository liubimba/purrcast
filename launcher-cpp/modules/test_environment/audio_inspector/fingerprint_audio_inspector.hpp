...
class fingerprint_audio_inspector : public i_audio_inspector
{
    ...
    void update_error_stats_(const verification_error error);

public:
    explicit fingerprint_audio_inspector(int sample_rate = 48000);
    ~fingerprint_audio_inspector() override;
    void add_reference(const audio_chunk& chunk, uint64_t seq) override;
    verification_report verify(const audio_chunk& chunk) override;

    [[nodiscard]] const verification_stats& get_stats() const override;
