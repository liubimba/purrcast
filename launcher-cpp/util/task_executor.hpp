...
template <typename T>
struct task_impl : i_task
{
    std::promise<T> promise;
    std::function<T(void)> func;
    std::shared_ptr<spdlog::logger> logger;

    task_impl(std::function<T(void)>&& func, const std::shared_ptr<spdlog::logger>& logger):
...
{
public:
    task_executor(const services* services, const std::string& id):
        services_(services)
    {
        logger_ = services->get<logger_factory>()->create("Executor-" + id);
        thread_ = std::thread(&task_executor::run_, this);
    }
    ...
    std::future<T> post(std::function<T(void)> func)
    {
        ...
        std::unique_lock<std::mutex> lock(mutex_);
        auto task = std::make_unique<task_impl<T>>(std::move(func), logger_);
...
private:
    ...
    std::queue<std::unique_ptr<i_task>> tasks_;
};
